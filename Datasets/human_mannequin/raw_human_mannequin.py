...
from util.densepose_util import IUV2UpperBodyImg, IUV2TorsoLeg, IUV2Img
from util.cv2_trans_util import get_inverse_trans
import cv2
import json

class RawHumanMannequin(data.Dataset):
    def __init__(self,path='./data/HumanMannequin/jin_raw_01',img_size=512,simplified_dp=True):
        self.simplified_dp=simplified_dp
        self.img_dir = path
    ...
    def __getitem__(self, index):
        ...
        if self.simplified_dp:
            dp_img = IUV2TorsoLeg(IUV)
        else:
            dp_img = IUV2Img(IUV)
        dp_img = cv2.resize(dp_img, (raw_w, raw_h))
    ...
    def get_item_for_test(self, index):
        garment_path = self.image_list[index]
        garment_img = np.array(Image.open(garment_path))
        raw_h,raw_w =self.raw_height, self.raw_width
        trans2roi_path = os.path.join(self.img_dir,
        ...
        new_trans2roi = trans2roi#self.randomaffine(trans2roi)
        ...
        if torch.cuda.is_available():
            ...
            roi_mask_img = roi_mask_img.cuda()
            roi_dp_img = roi_dp_img.cuda()
        return self._normalize(roi_garment_img),self._normalize(roi_vm_img),self._normalize(roi_dp_img),roi_mask_img, garment_img,vm_img,dp_img,mask_img
